...
class SecureMessengerClient:
    ...
    def handle_group_create_ok(self, message):
        """Обработка подтверждения создания группы от сервера"""
        logger.debug(f"handle_group_create_ok: Получено подтверждение создания группы")

        group_id = message.get('group_id')
        group_name = self.group_chats.get(group_id, {}).get('name')

        if not group_id:
            logger.warning(f"handle_group_create_ok: Не указан ID группы")
            return

        # Проверяем, существует ли уже группа с таким ID (на случай дублирования)
        if group_id not in self.group_chats:
            # Если группы нет, создаем ее локально (это может быть группа, созданная другим пользователем)
            if not group_name:
                group_name = message.get('message', '').replace('Группа создана успешно', '').strip()
                if not group_name:
                    group_name = f"Группа {group_id[:8]}"

            logger.debug(f"handle_group_create_ok: Создание локальной записи для группы {group_id}")

            self.group_chats[group_id] = {
                'name': group_name,
                'members': [self.username],  # Только создатель
                'admin': self.username,
                'symmetric_key': Fernet.generate_key(),  # Генерируем ключ
        ...
        # Ищем и выбираем созданную группу
        if self.current_tab == 'group':
            self.root.after(100, lambda: self.select_group_after_creation(group_id))

        logger.info(f"handle_group_create_ok: Группа {group_id} создана и подтверждена сервером")

    def select_group_after_creation(self, group_id):
        """Выбор созданной группы в списке после подтверждения"""
        for i in range(self.groups_listbox.size()):
            display_text = self.groups_listbox.get(i)
            # Извлекаем название группы из отображаемого текста
            match = re.match(r'^[👑👥Adm]\s+(.+?)(?:\s+\(\d+\))?$', display_text)
            if match:
                displayed_name = match.group(1)
                # Находим ID группы по имени
                for gid, info in self.group_chats.items():
                    if info['name'] == displayed_name and gid == group_id:
                        self.groups_listbox.selection_set(i)
                        self.groups_listbox.activate(i)
                        self.root.after(200, lambda: self.on_group_select())
                        logger.debug(f"select_group_after_creation: Выбрана созданная группа {displayed_name}")
                        return

    def request_group_list_from_server(self):
        """Запрос списка групп с сервера при загрузке клиента"""
        logger.debug(f"request_group_list_from_server: Запрос списка групп с сервера")

        if not self.connected:
            logger.warning(f"request_group_list_from_server: Нет подключения к серверу")
            return

        # В данном случае мы не можем напрямую запросить список групп,
        # так как в текущем протоколе нет такой команды.
        # Вместо этого, сервер сам отправит нам приглашения в группы при подключении.
        # Эти приглашения обрабатываются в handle_group_invite/handle_group_create

        logger.debug(f"request_group_list_from_server: Сервер сам отправит приглашения при подключении")
    ...
    def on_tab_changed(self, event):
        ...
        if tab_text == "Личные чаты":
           ...
           # self.group_manage_btn.config(state='disabled')
            ...
            # Если после загрузки групп нет активного чата, выбираем первую
            if not self.active_chat or self.active_chat_type != 'group':
                if self.groups_listbox.size() > 0:
                    logger.debug(f"on_tab_changed: Автоматически выбираем группу при переключении на вкладку")
                    self.root.after(100, lambda: self.auto_select_first_group_on_tab())
                else:
    ...
    def auto_select_first_group_on_tab(self):
        """Автоматический выбор первой группы при переключении на вкладку"""
        if self.groups_listbox.size() == 0:
            return
    ...
    def load_group_chats_list(self):
        ...
        # Сохраняем соответствие отображаемого текста и ID группы
        self.group_display_map = {}  # Добавим эту переменную в __init__
        ...
        for group_id, group_info in self.group_chats.items():
            ...
            self.group_display_map[display_text] = group_id

            logger.debug(f"load_group_chats_list: Добавлена группа: '{display_text}' -> ID: {group_id}")

        # Автоматически выбираем первую группу, если она есть
        if self.groups_listbox.size() > 0:
            self.groups_listbox.selection_set(0)
            self.groups_listbox.activate(0)
            logger.debug(f"load_group_chats_list: Автоматически выбрана первая группа")

            # Вызываем обработчик выбора группы
            self.root.after(100, self.on_group_select)

        logger.debug(f"load_group_chats_list: Загружено {len(self.group_chats)} групп")

    def auto_select_first_group(self, groups_loaded):
        """Автоматический выбор первой группы"""
        if not groups_loaded or self.groups_listbox.size() == 0:
            logger.debug(f"auto_select_first_group: Нет групп для выбора")
            return
        ...
        # Получаем информацию о первой группе
        group_id, group_name, display_text = groups_loaded[0]

        logger.debug(f"auto_select_first_group: Автоматически выбираем группу {group_id} ('{group_name}')")

        # Устанавливаем активный чат
        self.active_chat = group_id
        self.active_chat_type = 'group'
        logger.debug(f"auto_select_first_group: Активный чат установлен: {self.active_chat}, тип: {self.active_chat_type}")

        # Обновляем заголовок
        admin_marker = " (Adm)" if self.group_chats[group_id]['admin'] == self.username else ""
        ...
        self.message_entry.focus_set()
        logger.debug(f"auto_select_first_group: Группа '{group_name}' выбрана автоматически")
    ...
    def load_group_chat(self, group_id):
        ...
        logger.debug(f"load_group_chat: Активный чат: {self.active_chat}, Запрошенная группа: {group_id}")
        logger.debug(f"load_group_chat: Все группы: {list(self.group_chats.keys())}")
        ...
        if group_id in self.group_messages:
            ...
            logger.debug(f"load_group_chat: Загружено {len(messages)} сообщений для группы {group_id}")
            ...
            logger.debug(f"load_group_chat: Сообщения отображены для группы {group_id}")
        else:
            logger.debug(f"load_group_chat: Нет сообщений для группы {group_id}")

        self.chat_display.config(state='disabled')
        self.chat_display.yview(tk.END)

        # Добавим отладочную информацию в заголовок
        group_name = self.group_chats.get(group_id, {}).get('name', 'Неизвестно')
        logger.debug(f"load_group_chat: Заголовок должен быть: 'Группа: {group_name}'")
    ...
    def add_user_to_group(self):
        ...
        # Проверяем, есть ли у нас публичный ключ пользователя
        if username not in self.contacts:
            logger.warning(f"add_user_to_group: Нет публичного ключа для {username}, запрашиваем...")
            self.request_public_key(username)
            messagebox.showinfo("Информация", f"Сначала нужно получить ключ пользователя {username}")
            return

        try:
            # Получаем публичный ключ пользователя
            recipient_key = self.contacts[username]['public_key']

            # Генерируем новый ключ для группы
            new_key = Fernet.generate_key()
            group_info['symmetric_key'] = new_key

            # Шифруем ключ для нового участника
            ...
            # Подготавливаем зашифрованные ключи для отправки
            encrypted_keys = {}
            encrypted_keys[username] = encrypted_key_b64

            # Также шифруем для существующих участников
            for member in group_info['members']:
                if member != username and member in self.contacts:
                    try:
                        member_key = self.contacts[member]['public_key']
                        member_encrypted_key = member_key.encrypt(
                            new_key,
                            padding.OAEP(
                                mgf=padding.MGF1(algorithm=hashes.SHA256()),
                                algorithm=hashes.SHA256(),
                        ...
                        encrypted_keys[member] = base64.b64encode(member_encrypted_key).decode('utf-8')
                    except Exception as e:
                        logger.error(f"add_user_to_group: Ошибка шифрования для {member}: {e}")

            # Отправляем уведомление о добавлении
            self.send_group_member_added(group_id, username, encrypted_keys)
           ...
           # messagebox.showinfo("Успех", f"Пользователь {username} добавлен в группу")
            ...
            logger.error(f"add_user_to_group: Ошибка при добавлении пользователя: {e}")
    ...
    def show_group_management(self):
        ...
        if not group_info:
            logger.warning(f"show_group_management: Группа {group_id} не найдена")
            return
        ...
        # Информация о группе
        info_frame = ttk.LabelFrame(main_frame, text="Информация о группе", padding=10)
        info_frame.pack(fill=tk.X, pady=(0, 15))

        ttk.Label(info_frame, text=f"Администратор: {group_info['admin']}",
                 font=('Arial', 10)).pack(anchor=tk.W, pady=(0, 5))

        ttk.Label(info_frame, text=f"Участников: {len(group_info['members'])}",
                 font=('Arial', 10)).pack(anchor=tk.W)

        # Список участников
        members_frame = ttk.LabelFrame(main_frame, text="Участники группы", padding=10)
        ...
        for member in group_info['members']:
            ...
            elif member == group_info['admin']:
                members_listbox.insert(tk.END, f"{member} (👑 Админ)")
            else:
        ...
        # Кнопка "Выйти из группы" (доступна всем)
        def leave_group():
            if messagebox.askyesno("Подтверждение",
                                 f"Вы уверены, что хотите выйти из группы '{group_info['name']}'?"):
                self.leave_from_group(group_id)
                dialog.destroy()

        leave_btn = ttk.Button(btn_frame, text="Выйти из группы",
                              command=leave_group)
        leave_btn.pack(side=tk.LEFT, padx=(0, 10))

        # Кнопка "Удалить участника" (только для админа и не себя)
        def remove_selected_member():
            ...
            selected_text = members_listbox.get(selection[0])

            # Извлекаем имя пользователя из текста
            selected_member = selected_text
            if " (Вы)" in selected_text:
                selected_member = selected_text.replace(" (Вы)", "")
            elif " (👑 Админ)" in selected_text:
                selected_member = selected_text.replace(" (👑 Админ)", "")

            # Проверяем, не пытаемся ли удалить себя
            if selected_member == self.username:
                messagebox.showinfo("Информация", "Для выхода используйте кнопку 'Выйти из группы'")
                return

            # Проверяем, не пытаемся ли удалить админа (если мы не админ)
            if group_info['admin'] != self.username:
                messagebox.showwarning("Ошибка", "Только администратор может удалять участников")
                return
        ...
        # Показываем кнопку удаления только админу
        if group_info['admin'] == self.username:
            remove_btn = ttk.Button(btn_frame, text="Удалить выбранного",
                                   command=remove_selected_member)
    ...
    def leave_from_group(self, group_id):
        """Выход из группы"""
        logger.debug(f"leave_from_group: Выход из группы {group_id}")

        if group_id not in self.group_chats:
            logger.error(f"leave_from_group: Группа {group_id} не найдена")
            return
        ...
        # Отправляем запрос на выход из группы
        data = {
            'type': 'group_leave',
            'group_id': group_id,
            'timestamp': datetime.now().isoformat()
        ...
        try:
            self.safe_send(json.dumps(data).encode('utf-8'))
            logger.debug(f"leave_from_group: Запрос на выход отправлен")

            # Удаляем группу локально
            self.remove_group_locally(group_id)

        except Exception as e:
            logger.error(f"leave_from_group: Ошибка отправки запроса: {e}")
            messagebox.showerror("Ошибка", f"Не удалось выйти из группы: {str(e)}")

    def remove_group_locally(self, group_id):
        """Удаление группы из локального хранилища"""
        logger.debug(f"remove_group_locally: Удаление группы {group_id}")
        ...
        # Если эта группа была активной, сбрасываем активный чат
        ...
        logger.info(f"remove_group_locally: Группа {group_id} удалена локально")


    def remove_member_from_group(self, group_id, member_username):
        """Удаление участника из группы администратором"""
        ...
        data = {
            ...
            'admin': self.username,
            'encrypted_keys': {},  # Будет заполнено сервером
            'timestamp': datetime.now().isoformat()
        }

        try:
            self.safe_send(json.dumps(data).encode('utf-8'))
            logger.debug(f"remove_member_from_group: Запрос на удаление отправлен")

            if member_username in group_info['members']:
                group_info['members'].remove(member_username)
                logger.debug(f"remove_member_from_group: Участник {member_username} удален из локального списка")

        except Exception as e:
            logger.error(f"remove_member_from_group: Ошибка отправки запроса: {e}")
            messagebox.showerror("Ошибка", f"Не удалось удалить участника: {str(e)}")
    ...
    def send_group_message(self, message_text):
        ...
        if not self.check_group_key(group_id):
            logger.error(f"send_group_message: У группы {group_id} невалидный ключ")
            messagebox.showerror("Ошибка",
                f"У группы нет валидного ключа шифрования.\n"
                f"Попробуйте перезайти в приложение.")
            return
    ...
    def handle_group_info(self, message):
        """Обработка информации о группе от сервера"""
        logger.debug(f"handle_group_info: Обработка информации о группе")
        ...
        logger.debug(f"handle_group_info: Группа {group_id} '{group_name}', админ: {admin}, участников: {len(members)}")

        # Проверяем, существует ли уже такая группа
        if group_id in self.group_chats:
            logger.debug(f"handle_group_info: Группа {group_id} уже существует, обновляем информацию")
            # Обновляем информацию
            self.group_chats[group_id]['name'] = group_name
            self.group_chats[group_id]['admin'] = admin
            self.group_chats[group_id]['members'] = members
        else:
            logger.debug(f"handle_group_info: Создаем новую запись группы {group_id}")
            # Создаем новую группу
            self.group_chats[group_id] = {
                'name': group_name,
                'members': members,
                'admin': admin,
                'symmetric_key': None,  # Ключа пока нет
                'needs_key': True,  # Флаг, что нужен ключ
            ...
            system_msg = {
                'sender': 'Система',
                'text': f'Вы участник группы "{group_name}"',
                'timestamp': datetime.now().isoformat(),
                'system': True
            }
            ...
            logger.info(f"handle_group_info: Добавлена информация о группе {group_id}")
        ...
        # Запрашиваем ключ для группы (если его нет)
        if group_id in self.group_chats and self.group_chats[group_id].get('needs_key'):
            logger.debug(f"handle_group_info: Для группы {group_id} нужен ключ")
            # В текущей реализации ключ приходит только с приглашением (group_invite)
            # Пользователь должен запросить ключ у администратора или ждать перешифровки
    ...
    def process_server_message(self, message):
        ...
        if msg_type == 'group_create':
            ...
            self.handle_group_create_ok(message)
        elif msg_type == 'group_info':
            self.handle_group_info(message)
        elif msg_type == 'group_leave_ok':
            self.handle_group_leave_ok(message)
        elif msg_type == 'group_member_left':
            self.handle_group_member_left(message)
        else:
            # Обработка старых типов сообщений
            self.process_legacy_server_message(message)

    def handle_group_leave_ok(self, message):
        """Обработка подтверждения выхода из группы"""
        logger.debug(f"handle_group_leave_ok: Обработка подтверждения выхода из группы")

        group_id = message.get('group_id')
        group_name = message.get('group_name', f"Группа {group_id[:8]}" if group_id else "Неизвестная")
        msg_text = message.get('message', 'Вы вышли из группы')

        logger.info(f"handle_group_leave_ok: {msg_text}")
        ...
        self.root.after(0, lambda: messagebox.showinfo("Выход из группы", msg_text))

    def handle_group_member_left(self, message):
        """Обработка уведомления о выходе другого участника из группы"""
        logger.debug(f"handle_group_member_left: Обработка выхода участника из группы")

        group_id = message.get('group_id')
        left_member = message.get('left_member')
        admin = message.get('admin')
        encrypted_key = message.get('encrypted_key')

        logger.debug(f"handle_group_member_left: Участник {left_member} вышел из группы {group_id}")

        if group_id not in self.group_chats:
            logger.warning(f"handle_group_member_left: Группа {group_id} не найдена")
            return

        # Если это мы вышли, группа уже удалена
        if left_member == self.username:
            logger.debug(f"handle_group_member_left: Это наш выход, игнорируем")
            return

        # Обновляем ключ если он есть
        if encrypted_key:
            try:
                ...
                logger.debug(f"handle_group_member_left: Ключ группы обновлен")

            except Exception as e:
                logger.error(f"handle_group_member_left: Ошибка обновления ключа: {e}")

        # Удаляем участника из списка
        if left_member in self.group_chats[group_id]['members']:
            self.group_chats[group_id]['members'].remove(left_member)
            logger.debug(f"handle_group_member_left: Участник {left_member} удален из списка")
        ...
        system_msg = {
            'sender': 'Система',
            'text': f'Участник {left_member} вышел из группы',
            'timestamp': datetime.now().isoformat(),
            'system': True
        }
        ...
        logger.info(f"handle_group_member_left: Обработано уведомление о выходе {left_member} из {group_id}")
    ...
    def check_group_key(self, group_id):
        """Проверка наличия и валидности ключа группы"""
        logger.debug(f"check_group_key: Проверка ключа группы {group_id}")

        if group_id not in self.group_chats:
            logger.error(f"check_group_key: Группа {group_id} не найдена")
            return False

        group_info = self.group_chats[group_id]

        # Проверяем наличие ключа
        if 'symmetric_key' not in group_info or not group_info['symmetric_key']:
            logger.error(f"check_group_key: У группы {group_id} нет ключа!")
            return False

        key = group_info['symmetric_key']

        # Проверяем тип ключа (должен быть bytes)
        if not isinstance(key, bytes):
            logger.error(f"check_group_key: Ключ имеет неверный тип: {type(key)}")
            # Пробуем преобразовать
            if isinstance(key, str):
                try:
                    group_info['symmetric_key'] = key.encode('utf-8')
                    logger.debug(f"check_group_key: Ключ преобразован из str в bytes")
                    self.save_group_chats()
                    return True
                except:
                    pass
        ...
        # Проверяем длину ключа (Fernet ключ должен быть 32 байта в base64url)
        if len(key) != 44:  # 32 байта в base64url = 44 символа
            logger.warning(f"check_group_key: Длина ключа {len(key)} нестандартна")
            # Но все равно может работать если это валидный Fernet ключ

        logger.debug(f"check_group_key: Ключ группы {group_id} проверен: тип={type(key)}, длина={len(key)}")
        return True

    def handle_group_invite(self, message):
        """Обработка приглашения в группу (для НОВЫХ участников)"""
        logger.debug(f"handle_group_invite: Обработка приглашения в группу")

        group_id = message.get('group_id')
        group_name = message.get('group_name')
        admin = message.get('admin')
        encrypted_key = message.get('encrypted_key')  # Один ключ для нового участника

        logger.debug(f"handle_group_invite: Приглашение в группу {group_id} ('{group_name}') от {admin}")
        ...
        try:
            ...
            logger.debug(f"handle_group_invite: Ключ группы расшифрован, длина: {len(group_key)} байт")

            # Сохраняем/обновляем информацию о группе
            if group_id not in self.group_chats:
                # Новая группа для нас
                self.group_chats[group_id] = {
                    'name': group_name or f"Группа {group_id[:8]}",
                ...
                logger.debug(f"handle_group_invite: Создана новая группа {group_id}")
            else:
                # Группа уже существует (может быть при повторном приглашении)
                logger.debug(f"handle_group_invite: Группа {group_id} уже существует, обновляем ключ")
                self.group_chats[group_id]['symmetric_key'] = group_key
                # Обновляем участников
                if admin not in self.group_chats[group_id]['members']:
                    self.group_chats[group_id]['members'].append(admin)
                if self.username not in self.group_chats[group_id]['members']:
                    self.group_chats[group_id]['members'].append(self.username)

            # Создаем/обновляем запись для сообщений
            if group_id not in self.group_messages:
                self.group_messages[group_id] = []
            ...
            for msg in self.group_messages[group_id]:
                if (msg.get('system') and
                    f'Вы добавлены в группу' in msg.get('text', '') and
                    admin in msg.get('text', '')):
                    message_exists = True
                    break
            ...
            logger.info(f"handle_group_invite: Группа {group_id} создана/обновлена с ключом")
            ...
            logger.error(f"handle_group_invite: Ошибка обработки приглашения в группу: {e}")
    ...
    def handle_group_member_added(self, message):
        ...
        logger.debug(f"handle_group_member_added: Обработка добавления участника в группу")

        group_id = message.get('group_id')
        new_member = message.get('new_member')
        admin = message.get('admin')
        encrypted_keys = message.get('encrypted_keys')  # Это словарь с ключами для всех участников!

        logger.debug(f"handle_group_member_added: Добавление {new_member} в группу {group_id}")
        logger.debug(f"handle_group_member_added: encrypted_keys тип: {type(encrypted_keys)}, значение: {encrypted_keys}")

        if group_id not in self.group_chats:
            logger.warning(f"handle_group_member_added: Группа {group_id} не найдена")
            return

        # ВАЖНО: Это сообщение приходит СУЩЕСТВУЮЩИМ участникам, когда добавили нового
        # Новый участник получает group_invite отдельно

        # Если новый участник - это мы, значит нас добавили (это group_invite)
        if new_member == self.username:
            logger.debug(f"handle_group_member_added: Это приглашение для НАС, игнорируем - будет обработано в handle_group_invite")
            return

        # Значит, добавили кого-то другого, и нам прислали новый ключ
        logger.debug(f"handle_group_member_added: Нам (существующему участнику) прислали новый ключ после добавления {new_member}")

        # Проверяем, есть ли наш ключ в словаре
        if not encrypted_keys or not isinstance(encrypted_keys, dict):
            logger.error(f"handle_group_member_added: Некорректный формат encrypted_keys: {type(encrypted_keys)}")
            return

        if self.username not in encrypted_keys:
            logger.error(f"handle_group_member_added: Нет нашего ключа {self.username} в словаре. Доступные: {list(encrypted_keys.keys())}")
            return

        encrypted_key = encrypted_keys[self.username]
        logger.debug(f"handle_group_member_added: Найден наш зашифрованный ключ, длина: {len(encrypted_key)}")

        # Расшифровываем новый ключ группы
        try:
            encrypted_key_data = base64.b64decode(encrypted_key)
            new_group_key = self.private_key.decrypt(
                encrypted_key_data,
            ...
            logger.debug(f"handle_group_member_added: Новый ключ расшифрован, длина: {len(new_group_key)} байт")

            # Получаем старый ключ
            old_key = self.group_chats[group_id].get('symmetric_key')

            # ВАЖНО: Сохраняем старый ключ для расшифровки старых сообщений
            if old_key and old_key != new_group_key:
                self.group_chats[group_id]['previous_key'] = old_key
                logger.debug(f"handle_group_member_added: Сохранен старый ключ, длина: {len(old_key)}")

            # ОБНОВЛЯЕМ текущий ключ!
            self.group_chats[group_id]['symmetric_key'] = new_group_key
            logger.info(f"handle_group_member_added: Ключ группы {group_id} ОБНОВЛЕН! Старая длина: {len(old_key) if old_key else 0}, Новая: {len(new_group_key)}")

            # Добавляем нового участника в список, если его еще нет
            if new_member not in self.group_chats[group_id]['members']:
                self.group_chats[group_id]['members'].append(new_member)
                logger.debug(f"handle_group_member_added: Добавлен новый участник {new_member}")
            ...
            system_msg = {
                'sender': 'Система',
                'text': f'Участник {new_member} добавлен в группу. Ключ безопасности обновлен.',
                'timestamp': datetime.now().isoformat(),
                'system': True
            }
            ...
            self.root.after(0, lambda: messagebox.showinfo(
                "Обновление ключа",
                f"Ключ группы '{self.group_chats[group_id]['name']}' обновлен после добавления {new_member}"
            ))

            logger.info(f"handle_group_member_added: Ключ группы {group_id} успешно обновлен для {self.username}")

        except Exception as e:
            logger.error(f"handle_group_member_added: Ошибка обработки нового ключа: {e}")
            self.root.after(0, lambda: messagebox.showerror(
                "Ошибка ключа",
                f"Не удалось обновить ключ группы: {str(e)}"
            ))

    def update_group_members_after_removal(self, group_id, removed_member):
        """Обновление локального списка участников после удаления"""
        logger.debug(f"update_group_members_after_removal: Обновление участников группы {group_id}")
        ...
        if removed_member in self.group_chats[group_id]['members']:
            ...
            logger.debug(f"update_group_members_after_removal: Участник {removed_member} удален")
        ...
        system_msg = {
            'sender': 'Система',
            'text': f'Участник {removed_member} удален',
            'timestamp': datetime.now().isoformat(),
            'system': True
        }
        ...
        logger.info(f"update_group_members_after_removal: Список участников группы {group_id} обновлен")

    def handle_group_member_removed(self, message):
        """Обработка удаления участника из группы"""
        logger.debug(f"handle_group_member_removed: Обработка уведомления об удалении участника")
        ...
        logger.debug(f"handle_group_member_removed: Участник {removed_member} удален из группы {group_id} админом {admin}")
        ...
        if removed_member == self.username:
            logger.debug(f"handle_group_member_removed: Нас удалили из группы {group_id}")
            self.remove_group_locally(group_id)
            return
        ...
        if encrypted_key:
            try:
                ...
                # Обновляем ключ
                self.group_chats[group_id]['symmetric_key'] = new_group_key
                logger.debug(f"handle_group_member_removed: Ключ группы обновлен")

            except Exception as e:
                logger.error(f"handle_group_member_removed: Ошибка обновления ключа: {e}")

        # Удаляем участника из списка
        if removed_member in self.group_chats[group_id]['members']:
            self.group_chats[group_id]['members'].remove(removed_member)
            logger.debug(f"handle_group_member_removed: Участник {removed_member} удален из списка")
        ...
        system_msg = {
            'sender': 'Система',
            'text': f'Участник {removed_member} удален администратором {admin}',
            'timestamp': datetime.now().isoformat(),
            'system': True
        }
